...
def main():
    q0, p0= 1.2, 0.0
    pend=SimplePendulum(m=1.0, l=1.0, g=9.81)
    H0=pend.hamiltonian(np.array([q0]), np.array([p0]))[0]
    ...
    csv_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'tables', 'comparison_pendulum.csv')
    ...
    ax.set_ylabel('H(q,p)')
    ax.set_title(f' energy comparison on Pendulum for the four architectures, {TLONG}s rollout')
    ax.legend(loc='best', fontsize=9)
    ax.grid(alpha=.3)
    plt.tight_layout()
    fig_path = os.path.join(os.path.dirname(__file__), '..', 'results', 'figures', 'comparison_pendulum.png')
