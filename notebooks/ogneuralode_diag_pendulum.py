"""
diagnoistic for ogneuralode on longer rollouts for the pendulum specifically
same trained on 10 steps and .1s integration
"""
...
CHECKPOINTS = os.path.join(os.path.dirname(__file__), '..', 'results', 'checkpoints', 'neural_ode_pendulum.pt')
T_LONG = 50.0
N_POINTS_LONG = 5000
...
def oracle_of_truth_trajectory(q0, p0, t_eval):
    #generate the true pendulum trajectory for comparison
...
def main():
    ...
    #initial condition - nonlinear regime, pure release at ~69deg
    q0, p0 = 1.2, 0.0
    pend = SimplePendulum(m=1.0, l=1.0, g=9.81)
    ...
    plt.tight_layout()
    out1= os.path.join(os.path.dirname(__file__), '..', 'results', 'figures', 'neural_ode_pendulum_phase_portraits.png')
    ...
    ax.grid(alpha=.3)
    plt.tight_layout()
    out2=os.path.join(os.path.dirname(__file__), '..', 'results', 'figures', 'neural_ode_pendulum_energydrift.png')
